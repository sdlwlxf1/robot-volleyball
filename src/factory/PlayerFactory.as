package factory
{
	...
	public class PlayerFactory extends Object
	{
		...
		private var _particleMoveFog1:MovieClipParticle;
		...
		public function createPlayer(playerData:PlayerData, factory:StarlingFactory):Player
		{
			...
			//初始化主角
			_player = new Player(_playerData.name, {x: _playerData.startX, y: _playerData.startY, width: _viewSprite.width / 2, height: _viewSprite.height + 30, offsetY: 0, offsetX: 15, view: _armature, registration: "topLeft", data: _playerData});
			...
			{
				_arm1Display = _armature.getBone("outerarm_lower").display as DisplayObject;
				_arm2Display = _armature.getBone("innerarm_lower").display as DisplayObject;
			}
			
			_arm1 = new Hand("outerarm_lower", {player: _player, invertedViewAndBody: _player.invertedViewAndBody, width: _arm1Display.width * _playerData.scale, height: _arm1Display.height * 1.5 * _playerData.scale, followView: _arm1Display /*, view:SpriteDebugArt*/});
			CitrusEngine.getInstance().state.add(_arm1);
			
			_arm2 = new Hand("innerarm_lower", {player: _player, invertedViewAndBody: _player.invertedViewAndBody, width: _arm2Display.width * _playerData.scale, height: _arm2Display.height * _playerData.scale, followView: _arm2Display /*, view:SpriteDebugArt*/});
			...
			_particleFactory.object = _player;			
			_player.particleFactory = _particleFactory;	
			
			var fog1PoolParticle:PoolParticle = new PoolParticle(_particleFactory.createFog1, null, 30, 40);			
			_player.fog1PoolParticle = fog1PoolParticle;
			
			var fog2PoolParticle:PoolParticle = new PoolParticle(_particleFactory.createFog2, null, 30, 40);			
			_player.fog2PoolParticle = fog2PoolParticle;
			...
			_player.particleHandHit1 = _particleFactory.createMovieClipParticle(ParticleFactory.HAND_HIT_1, _player);

			_player.particleHandHit2 = _particleFactory.createMovieClipParticle(ParticleFactory.HAND_HIT_2, _player);		
			
			_player.particleFog3 = _particleFactory.createMovieClipParticle(ParticleFactory.FOG_3, _player);
			
			_player.particleJump1 = _particleFactory.createMovieClipParticle(ParticleFactory.JUMP_1, _player);
			
			//_player.particleJump1.movieClip.color = 0x000000;
			
			
			
			_player.canAutoJump = playerData.canAutoJump;
			
			_player.canAutoAccelate = playerData.canAutoAccelate;
