package objectPools {
	...
	public class PoolParticle {
		...
		/** Objects in the pool. */
		private var list:Vector.<MovieClipParticle> = new Vector.<MovieClipParticle>();
		
		/** If this pool has been disposed. */
		private var disposed:Boolean = false;
		
		
		private var movieClipParticle:MovieClipParticle;
		...
		{
			list[length++] = createParticle();		
			size++;
		}
		
		private function createParticle():MovieClipParticle
		{
			movieClipParticle = create();
			movieClipParticle.poolParticle = this;
			return movieClipParticle;
		}
		 ...
		 *
		 */
		public function checkOut():MovieClipParticle
		{
			if(length == 0) {
				if(size < maxSize) {
					size++;
		 ...
		 *
		 */
		public function checkIn(item:MovieClipParticle):void
		{
