	...
	public class Player extends DynamicObject
	{
		...
		override public function update(timeDelta:Number):void
		{
			...
			if (view && upperHandBody)
			{
				...
				upperHandView /*display*/.x = upperHandBodyPosition.x * 30;
				upperHandView /*display*/.y = upperHandBodyPosition.y * 30;
				upperHandView /*display*/.rotation = _invertedBody ? -upperHandBody.GetAngle() : upperHandBody.GetAngle() /* * 180 / Math.PI*/;
			}
			//updateHand();
		}
		...
		override public function set y(value:Number):void 
		{
			...
			if (upperHandBody) 
			{
				var pos:b2Vec2 = upperHandBody.GetPosition();
				pos.y = _y;
				upperHandBody.SetTransform(new b2Transform(pos, b2Mat22.FromAngle(upperHandBody.GetAngle())));
			}
		}
		...
		{
			_box2D.world.DestroyJoint(upperHandJoint);
			_box2D.world.DestroyJoint(underHandJoint);
			_box2D.world.DestroyJoint(hitAreaJoint);
			//_box2D.world.DestroyJoint(moveAreaJoint);
			_box2D.world.DestroyBody(upperHandBody);
			_box2D.world.DestroyBody(underHandBody);
